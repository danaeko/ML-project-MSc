...
columns_names_csv=['Model', "Method",'Best Features', 'Performance full', 'Performance selected']
...
print("results with dummy classifier %.3f"%dummy_clf.score(X_test, y_test))
...
## Load the saved models
    ...
    for model in models:
        ...
        print(labels[:12][np.array(indices).reshape((12))])
        # Project features

        sel_fea=labels[:12][np.array(indices).reshape((12))]
        ...
        X_train_red=X_train[0:,np.array(indices).reshape(12)]


        model.fit(X_train_red,y_train)

        
        X_test_red=X_test[:,np.array(indices).reshape(12)]
        ...
        str_f=""
        for feat in labels[:12][np.array(indices).reshape((12))]:
        ...
        results_list_un_c.append(str_f)
        ...
        results_list_rfe_c.append(str_f)
